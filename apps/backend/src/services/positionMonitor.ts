import { lastPriceMap, priceMap, trailingStopLossOrdersMap, dirtyOrders } from "../data/store";
import { checkBuyLiquidations, checkSellLiquidations, closeOrder } from "../utils/tradeUtils";
import { SUPPORTED_ASSETS } from "../constants/envConstants";
import { logger } from "../utils/logger";
import { prisma } from "database";
import type { Order, Asset, reasonForClose, redisPriceData } from "../constants/types";
...
export const startLiquidationChecker = () => {
  setInterval(async () => {
    for (const asset of SUPPORTED_ASSETS) {
      ...
      if (!latestPrice) {
        logger.info(`[Liquidation Check] ${asset}: No price data available yet`);
        continue;
      };
      ...
      if (latestPrice.ask > lastProcessed.ask) {
        await checkSellLiquidations(asset, latestPrice.ask, lastProcessed.ask);
      } 
...
export const updateTrailingStopLoss = async (priceData: redisPriceData, asset: Asset) => {
  ...
  for (const order of orders) {
    if (order.trailingStopLoss && order.trailingStopLoss.enabled) {
      if (order.type === "BUY") {
        ...
        if (!order.trailingStopLoss.highestPrice || priceData.bid > order.trailingStopLoss.highestPrice) {
          order.trailingStopLoss.highestPrice = priceData.bid;
          order.stopLoss = priceData.bid - order.trailingStopLoss.trailingDistance;
          dirtyOrders.add(order.orderId); // Mark order as dirty for DB update
          console.log(`Updated trailing stop loss for BUY order ${order.orderId}: new stop loss at ${order.stopLoss / 1000000} USD`);
        }
      ...
      if (!order.trailingStopLoss!.lowestPrice || priceData.ask < order.trailingStopLoss!.lowestPrice) {
        order.trailingStopLoss!.lowestPrice = priceData.ask;
        order.stopLoss = priceData.ask + order.trailingStopLoss!.trailingDistance;
        dirtyOrders.add(order.orderId); // Mark order as dirty for DB update
        console.log(`Updated trailing stop loss for SELL order ${order.orderId}: new stop loss at ${order.stopLoss / 1000000} USD`);
  ...
  if(order.type === "BUY") {
    await closeOrder(order.orderId, order.userId, order.type, priceData.bid,order, "stop_loss" as reasonForClose);
  } else {
     await closeOrder(order.orderId, order.userId, order.type, priceData.ask,order, "stop_loss" as reasonForClose);
  }
        ...
        await prisma.activeOrder.update({
          ...
          data: {
            ...
            trailingStopLoss: {
              ...
              highestPrice: order.trailingStopLoss!.highestPrice,
              lowestPrice: order.trailingStopLoss!.lowestPrice,
            },
          },
        });
      } catch (err) {
        dirtyOrders.add(order.orderId); // Re-add to dirty set for retry in next cycle
        logger.error(`Error updating TSL for order ${order.orderId} in DB:`, err);
      }
    })
  )
}, 5000);
